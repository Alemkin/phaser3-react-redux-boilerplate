...
function create (game) {
  ...
  game.map = game.make.tilemap({ key: Assets.TUXEMON_JSON })

  const tileset = game.map.addTilesetImage('tuxmon-sample-32px-extruded', Assets.TUXEMON)

  const belowLayer = game.map.createStaticLayer('Below Player', tileset, 0, 0)
  const worldLayer = game.map.createStaticLayer('World', tileset, 0, 0)
  const aboveLayer = game.map.createStaticLayer('Above Player', tileset, 0, 0)
